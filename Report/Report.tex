...
We can uniquely define a helical compression spring by a set of design parameters, which then determine physical attributes of the spring~\cite{Massad2015}. Figures~\ref{fig:Description1}~and~\ref{fig:Description2} provide visual representations for a few of the basic parameters, and a complete list of parameters and attributes follows. 		 
		...
		\begin{description}[leftmargin=!,labelwidth=\widthof{\bfseries Young's Modulus}]
			\item [Inner Diameter] Denoted $d_{i}$, this is the diameter of the circle formed by the spring's interior. See Figure~\ref{fig:Description2}.
			
			\item [Outer Diameter] Denoted $d_{\text{o}}$, this is the diameter of the circle formed by the spring including the wire. See Figure~\ref{fig:Description1}.
			
			\item [Wire Diameter] Denoted $d_{\text{w}}$, this is the diameter of the circular wire used to form the spring.
			
			\item[Total Coils] Denoted $N_{\text{t}}$, this is the total number of 360 degree rotations in the spring. We allow for partial rotations.
			
			\item[Active Coils] Denoted $N_{\text{a}}$, this is the number of coils that do not touch any other coils. The number of active coils depends on $N_{t}$ and the spring's end conditions. If the end condition is \textit{closed} the terminal coils of the spring are welded to their adjacent coils and $N_{a} = N_{t}-2$. If the end condition is \textit{open}, the terminal coils of the spring are free and $N_{a} = N_{t}-1$. Note this relationship creates a lower bound on the total number of coils.

			\item[Free Length] Denoted $L_{\text{free}}$, this is the spring's length without any force acting on it, i.e., the spring's natural length.
			
			\item[Solid Length] Denoted $L_{\text{solid}}$, this is the spring's length when its coils are completely compressed. The solid length depends on $N_{\text{t}}$ and $d_{\text{w}}$.
			
			\item[Open Length] Denoted $L_{\text{open}}$, this is the spring's resting length in its application. In the acceleration switch, for example, it is the length of the spring in the switch before the switch is used.
			
			\item[Hard Length] Denoted $L_{\text{hard}}$, this is the minimum length a spring can compress to under a specified force. For the acceleration switch example, the spring cannot compression beyond a certain point under the applied forces because additional compression would reopen the circuit. 
			
			\item[Open Force] Denoted $F_{\text{open}}$, this is the force applied to the spring while it is in the open position. For the acceleration switch, this is the applied force on the spring when the switch is not in use.
					
			\item[Pitch] Denoted $p$, this is the distance between coils. The pitch depends on $d_{\text{w}}$, $L_{\text{free}}$, $N_{\text{a}}$, and the spring's end conditions. See Figure~\ref{fig:Description1}.			
			
			\item[Young's Modulus] Denoted $E$, this describes the spring's response to uniaxial stress and helps to define the spring's rigidity. Young's modulus is also know as the modulus of elasticity.
			
			\item[Poisson Ratio] Denoted $\nu$, this is a measurement of how much the spring deforms in two dimensions when it is compressed in the third.
			
			\item[Shear Modulus] Denoted $G$, this describes the spring's response to shear stress and helps to define the spring's rigidity. The shear modulus is also known as the modulus of rigidity and depends on $E$ and $\nu$. 
		...
		\noindent\textbf{Spring Attributes:}
			\begin{description}
				...
				The spring rate is a measurement of the rigidity or stiffness of the spring. 
			
			\item[Spring Index]\begin{equation}C = \frac{d_{\text{i}}}{d_{\text{w}}} + 1.\end{equation}
				The spring index gives a measurement of the ratio between the spring's inner diameter and wire diameter and often has a direction relationship to the manufacturing cost for the spring~\cite{SpringIndex}.
			
			\item[Preload Force]\begin{equation*} F_{\text{open}} = (L_{\text{free}}-L_{\text{hard}})\frac{G}{8N_{\text{a}}}\frac{d_{\text{w}}^{4}}{(d_{\text{i}} + d_{\text{w}})^{3}} \end{equation*}
				Preload Force is the force applied to the spring while it is in the open position. For the acceleration switch example, this is the applied force on the spring when the switch is not in use.
				
			\item[Coil Binding Gap]\begin{equation} g = \frac{L_{\text{hard}} - L_{\text{solid}}(d_{\text{w}},N_{\text{a}})}{N_{\text{t}} - 1}\end{equation}		
			 ...
			 \item[Buckling Slenderness Ratio]\begin{equation*} \lambda = \frac{L_{\text{free}}}{d_{\text{i}} + d_{\text{w}}} \end{equation*}
			 	The buckling slenderness ratio is a measurement of the spring's susceptibility to buckling as it is compressed.
...
d_{\text{w}} + 3 d_{\text{w}}^{2})}{d_{\text{i}}(d_{\text{i}}+d_{\text{w}})^{3}}\right]\end{equation}
				Shear strength is a measurement of the spring's ability to withstand shear stress. 
			
			\item[Diametral Expansion]\begin{equation} d_{\text{expand}} = d_{\text{w}} + \sqrt{(d_{\text{i}} + d_{\text{w}})^{2} + \frac{p^{2} - d_{\text{w}}}{\pi^{2}}}
			\end{equation}
				The diametral expansion is the measurement of how much the diameter of the spring increases as the spring is compressed. Note that the discriminant of the square root is necessarily positive, so diametral expansion can put additional constraints on the relationship between some parameters.	
			
			\end{description}

This list of parameters and attributes is not exhaustive, but it does demonstrate the inherent complexities found in spring design. 
