...
Generally, the user would like to optimize some of the spring attributes introduced in Section \label{sec:Springs}, while satisfying the problem's physical design constraints.  In the case of an acceleration switch, the user would like to achieve the smallest possible spring rate and index, while satisfying the physical dimensions of the switch device.  Thus, the next step in our workflow is to take the user input and formulate an optimization problem that includes an objective, a set of design variables and constraints.  

In order to run the design analysis, we have placed some restrictions on the form of this optimization problem.  First, the optimization's objective function must be specified as a weighted sum of minimums and maximums.  While other methods for multicriteria optimization exist, including these in our scheme has been left to future work.

The second restriction, based on discussions with engineers experienced with spring design, is to reduce the set of design variables and optimization attributes to those in Table - x.
...
\begin{center}
	 ...
	 \hline
		...
		Total spring coil ($N_{\text{t}}$)    & Outer Diameter ($d_{\text{o}}$) \\
		Spring's inner diameter ($d_{\text{i}}$)    & Coil Binding Gap ($g$) \\
		Spring's width diameter ($d_{\text{w}}$)    & Shear Strength ($UTS$) \\
		Shear modulus  ($G$)        & Stress Relaxation ($R$) \\
\hline
	 \end{tabular}
\end{center}

The decision to limit the space of design variables and objective functions is not a limitation of this method.  Rather, it represents the set of options currently available in our design library.  If the user wishes to constrain or optimize an attribute that is not part of the existing library, the flexibility is there to add it.

An example of this flexibility was our team's integration of stress relaxation into the design process - a feature, which to our knowledge, no other spring design system has.  Indeed, after our team found a mathematical formulation for stress relaxation(see Appendix-xx for details), the function library was extended, allowing the user to incorporate relaxation into any problem formulation.

In addition to the constraint and objective libraries, we employed an object-oriented approach to our software design to maximize the flexibility in our workflow.  Object-oriented programming is an approach to software design that centers around classes.  Classes are simply data containers capable of performing a predefined set of tasks.  For example, our program has a \textbf{Constraint} class that includes the constraint's function, and a way to check whether the constraint is violated at a given point.
...
Once the user has formulated an optimization problem, the next step is to determine whether the problem is in fact feasible.  Feasibility is assessed based on the user's specified system of constraints.  Since each constraint in the library is written in terms of the permissible design variables, a point's feasibility can be assessed by simplifying testing whether all points are satisfied.

To assess feasibility, we use a Latin hypercube sampling method.  This method is designed to ensure sampling coverage includes the entire domain.  A sampling based approach is used because the user defined constraints can be very general in nature (for example, they can be linear or non-linear).  

At this point in the workflow, if a feasible point cannot be identified, the algorithm will terminate and inform the user that their problem may be poorly specified.  From here, the user can try increasing the sampling size to search for a feasible solution, or reconsider their problem's formulation.

To assist the user's understanding of the problem, a visualization tool has also been developed`.  As illustrated in the case studies, a plot with two or three state variables can be displayed, highlighting the problem's overall feasible region and the contributions from each individual constraint.  This will allow the user to identify the nature of the constraints in their problem.
